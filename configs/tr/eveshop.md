...
**`matching/analysis/build_clean.py` → `SUPPLEMENT_JOINS`** — `VALUE_CLEAN_OVERRIDES`'a paralel, yeni bir istisna sözlüğü: `clean_{site}` kurulurken (varsa) `eveshop_html_supplement`'ı `id = product_id` üzerinden `LEFT JOIN` edip `ean_html`/`eve_price_html`/`qty_html`/`html_fetch_date` kolonlarını ekliyor. Supplement tablosu henüz üretilmemişse (`_table_exists` kontrolü) sessizce atlanıyor — `clean_eveshop` yine kurulur, sadece o 4 kolon NULL kalır (script'in hiç çalıştırılmamış olması pipeline'ı kırmıyor).
